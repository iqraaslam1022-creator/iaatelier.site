    ...
    <section className="py-28 lg:py-36" style={{ backgroundColor: "#1A1A1A" }}>
      <div className="max-w-4xl mx-auto px-6 lg:px-10 text-center">
        ...
        <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="font-display text-xl lg:text-2xl text-white leading-relaxed mb-8 italic">"{item.review_text}"</p>
        ...
        <div className="flex justify-center gap-2 mt-8">
          {items.map((_, i) => (
            <button key={i} onClick={() => setIdx(i)} className={`w-1.5 h-1.5 rounded-full transition-all ${i === idx ? "bg-[#D4AF37] w-4" : "bg-white/20"}`} />
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    <section className="py-28 lg:py-36" style={{ backgroundColor: "#F5F5F0" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <p className="text-[#6B6B6B] font-light mb-16 max-w-lg">{data.subtitle}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {data.packages.map((pkg, i) => {
            ...
            return (
              <div key={i} className={`lux-card rounded-sm p-8 flex flex-col relative ${pkg.is_featured ? "ring-2 ring-[#B8972E]" : ""}`}
                ...
                <Link to={pkg.cta_link || "/contact"} className={`text-center py-3.5 rounded-sm text-xs tracking-widest uppercase font-semibold transition-all ${pkg.is_featured ? "bg-[#B8972E] text-white hover:bg-[#D4AF37]" : "bg-[#1A1A1A] text-white hover:bg-[#B8972E]"}`}>
                  {pkg.cta_text}
                </Link>
              </div>
            );
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-10 w-full pt-32 pb-24">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 1.1 }}
            ...
            <Link to="/portfolio" className="btn-gold rounded-sm inline-flex items-center gap-2">
              <span>{h.hero.ctaPrimary}</span>
            </Link>
            <Link to="/contact" className="border border-white/50 text-white hover:bg-white hover:text-[#1A1A1A] transition-all px-8 py-3.5 text-xs tracking-widest uppercase font-semibold rounded-sm">
              {h.hero.ctaSecondary}
            </Link>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {h.services.items.map((svc) => (
              <div key={svc.num} className="service-item lux-card rounded-sm p-7 group" style={{ opacity: 0 }}>
